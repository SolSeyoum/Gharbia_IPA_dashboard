...
units = {
    ...
    "total seasonal biomass production": "ton",
    "seasonal yield": "ton/ha",
    "crop water productivity": "kg/m³",
}

crop_calendar = {
    "Maize": "May to September",
    "Cotton": "May to September",
    "Vegetables": "May to September",
    "Other_crops": "May to September",
    "Rice": "May to September",
    "Trees": "May to April",
    "Wheat": "October to April",
    "Clover": "October to April",
    "AlfaAlfa": "October to April",
    "SugarBeet": "October to April",
    "winter Vegetables": "October to April",
    "winter Other_crops": "October to April",
    "Medicinal plants": "October to April",
}
...
def set_page_container_style(
    ...
    # Page configuration
    st.set_page_config(
        page_title=f"{Scheem_name} Irrigation Scheme Irrigation Performance Indicators Dashboard",
        page_icon="📈🌿",
        layout="wide",
...
# Load data
@st.cache_data(ttl=300)
def read_df_and_geo(Scheem_name, selected_season, selected_crop):
    dfm = pd.read_csv(
        rf"data/{Scheem_name}_IPA_statistic_{selected_crop}_{selected_season}.csv"
    )
    with open(rf"data/{Scheem_name}_IR.json") as response:
...
@st.cache_data(ttl=300)
def read_crop_area_df(selected_season):
    df = pd.read_csv(rf"data/crop_type_{selected_season}_precent.csv")
    return df


@st.cache_data(ttl=300)
def extract_crop_list(contains, pattern):
    """
    Select filenames containing a given string and extract
    the substring after the last underscore.
    """
    files = glob.glob(os.path.join(r"data", pattern))
    grouped_data = {}
    for f in files:
        name = os.path.basename(f)
        if contains in name:
            stem = os.path.splitext(name)[0]
            crop_name, season = stem.rsplit("_", 1)
            before, sep, crop_name = crop_name.partition(contains + "_")

            # Initialize season key if not exists
            if season not in grouped_data:
                grouped_data[season] = []

            # Add base name to that season
            if crop_name not in grouped_data[season]:
                grouped_data[season].append(crop_name)
    return grouped_data
...
def make_folium_choropleth(geo, indicator, df, col_name):
    ...
    if "section" in df.columns:
        ...
        geo["ch_name"] = "Zemam Sections"
    else:
        fields = [col_name, indicator]
        aliases = ["Division:", f"{ylable}:"]
        geo["ch_name"] = "Zemam Divisions"
...
def calculate_indicator_difference(input_df, indicator, input_season):
    ...
    previous_season_data = input_df[
        input_df["season"] == input_season - 1
    ].reset_index()
    selected_season_data["indicator_difference"] = selected_season_data[indicator].sub(
        previous_season_data[indicator], fill_value=0
    )
    return pd.concat(
        [
            selected_season_data["division"],
            selected_season_data[indicator],
            selected_season_data.indicator_difference,
        ],
        axis=1,
    ).sort_values(by="indicator_difference", ascending=False)


def history_df(df1, df2, idx_col, selected_indicator):
    d2 = df1.pivot(
        index=idx_col, columns="season", values=selected_indicator
    ).reset_index()
...
def alt_bar_chart(df, indicator, col_name, season_year, crop_season):
    xlable, text = indicator_title(indicator, stat_dict)
    df = df.round(2)
    indicator_name = indicator.replace("_", " ")
    plot_title = f"{indicator_name.title()} in {crop_season} of {str(season_year)}"
        ...
        .encode(
            y=alt.Y(
                ...
                sort=alt.EncodingSortField(
                    field="indicator", op="count", order="descending"
                ),
                title=area_id,
            ),  # Rename Y-axis
...
def make_alt_linechart(
    df_org, indicator, col_name, season, selected_section, selected_division
):
    ylable, text = indicator_title(indicator, stat_dict)
    df = df_org.copy()
    df["year"] = df["season_year"].str.split("-").str[1]
    ...
    # plot_title = f"{text.title()} per {area_id} for the past years of {season} season"
    y_title = f"{ylable.title()} [{units[ylable]}]"

    # title = f'{indicator_name.title()} per {area_id}s for the past seasons'

    if selected_section is not None:
        plot_title = f"{text.title()} per {area_id} of {selected_division} division for the past years of {season} season"
        ...
        chart = (
            ...
            .mark_line(size=3)
            .encode(
                x=alt.X("year:T", title="Season year", axis=alt.Axis(tickCount="year")),
                y=alt.Y(
                    f"{indicator}:Q",
                    title=y_title,
                ...
                color=alt.Color(
                    f"{col_name}:N", title=area_id, legend=alt.Legend(orient="top")
                ),
                opacity=alt.condition(
                    alt.datum.section
                    == selected_section,  # Highlight selected category
                ...
                tooltip=[
                    ...
                    alt.Tooltip(
                        f"{indicator}:Q", title=indicator, format=".2f"
                    ),  # Format Value as decimal with 2 digits
                ],
            )
            .properties(
        ...
        plot_title = f"{text.title()} per {area_id} of the scheme for the past years of {season} season"
        chart = (
            alt.Chart(df)
            .mark_line()
            .encode(
...
def alt_line_chart(df, indicator):
    ...
    df2 = (
        df.assign(season=pd.to_datetime(df["season"], format="%Y"))
    ...
    chart = (
        ...
        .encode(
            ...
            color=alt.Color(
                "variable:N", title="Point", legend=alt.Legend(orient="right")
            ),
            tooltip=[
                # alt.Tooltip(f'{col_name}:N', title=area_id),
...
def get_stats(_data):
    ...
    stats = {
        ...
        "25% quantile": _data.quantile(
            0.25, dim=["lat", "lon"], method="linear"
        ).drop_vars("quantile"),
        "75% quantile": _data.quantile(
            0.75, dim=["lat", "lon"], method="linear"
        ).drop_vars("quantile"),
    }

    # pd.DataFrame.from_dict(d)
    df_stat = pd.DataFrame.from_dict(
        {k: v.values.item() for k, v in stats.items()},
        orient="index",
        columns=["Values"],
    ).round(2)
...
def create_folium_map(data, geo, bounds, crs, variable):
    ...
    add_image_overlay(
        data, [[bottom, left], [top, right]], colors, variable, folium_map
    )
    ...
    click_marker = folium.Marker(
        location=[0, 0], popup="Click on the map", icon=folium.Icon(color="red")
    )
...
def plotly_pie_chart(dfca, name, year, crop_season):
    df = dfca.melt(
    ...
    fig.update_traces(
        hole=0.3, textposition="inside", textinfo="percent+label", textfont_size=16
    )
    fig.update_layout(showlegend=False)

    title = f"Percentage of Area covered by each crop type for {name} in {crop_season} of {year}"
    return fig, title
