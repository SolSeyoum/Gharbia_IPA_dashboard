...
with st.sidebar:
    ...
    # crop_lst = ["Maize", "Cotton", "Vegetables", "other_crops", "Rice", "Trees"]
    ...
    # st.write(crop_lst)
    dfm, geo = cm.read_df_and_geo(Scheem_name, selected_season, selected_crop)
    dfc = cm.read_crop_area_df(selected_season)
    crop_season = dfm.season_year.unique()[0]
    st.write(
        f"The selected crop :blue[{selected_crop}] is :blue[{selected_season}] season crop. The {selected_season} season runs from months of :blue[{cm.crop_calendar[selected_crop]}]."
    )
    season_year_list = list(dfm.season_year.unique())[::-1]
    ll = list(dfm.columns.unique())[4:][::-1]
    ...
    selected_season_year = st.selectbox(
        "Select a year of the season",
        season_year_list,
        index=0,
        help="Choose the year of the season to visualize",
    )

    dfm_season_year = dfm[dfm.season_year == selected_season_year]
    dfm_season_year = dfm_season_year.dropna()
    if dfm_season_year.empty:
        st.write("The :red[selected crop is not available for the selected season].")
    ...
    df_selected = dfm[dfm.season_year == selected_season_year][
        ["division", selected_indicator]
    ]
    df_selected_sorted = df_selected.sort_values(by=selected_indicator, ascending=False)

    # aggregate by divisions
    df_division = df_selected_sorted.groupby("division").agg(
        {selected_indicator: "mean"}
    )  # .rename(columns=d)
    df_division = df_division.sort_values(
    ...
    with st.expander("ℹ️ About the Indicator Map"):
        st.markdown(f"""
        This Indicator Map provides view of the Irrigation Performance Indicators (IPA) for {Scheem_name} Irrigation Scheme.
...
if st.session_state.selected_division is not None:
    ...
    filtered = [
        ...
        if sgeo["properties"]["division"] in selected_poly
    ]
    ...
    df_section = dfm[dfm.season_year == selected_season_year][
        ["division", "section", selected_indicator]
    ]
    ...
    # st.write(df_section_division)
    col_name = df_section_division.columns[1]

    # st.write(col_name)
    geo2plot = filtered_geojson
    df_map = df_section_division
    dfm_var = dfm[["season_year", "division", "section", selected_indicator]]
    df_chart = dfm_var.loc[dfm_var["division"] == selected_poly]

    selected_sections_ids = [
        feature["properties"]["ObjectID"] for feature in filtered_geojson["features"]
    ]
    dfca = (
        dfc[
            (dfc["season_year"] == selected_season_year)
    ...
    dfm_var = dfm[["season_year", "division", selected_indicator]].groupby(
        ["season_year", "division"]
    )
    df_chart = dfm_var.agg({selected_indicator: "mean"}).reset_index()

    dfca = (
        dfc[(dfc["season_year"] == selected_season_year)]
        .dropna(axis=1, how="any")
        # .drop(columns=["season"])
        .mean(numeric_only=True)
        .to_frame()
...
# st.write(st.session_state.selected_division)
# st.write(st.session_state.selected_section)
line_chart, title_line_chart = cm.make_alt_linechart(
    df_chart,
    selected_indicator,
    col_name,
    crop_season,
...
title = f'<p style="font:Courier; color:gray; font-size: 20px;">{title_line_chart}</p>'

selected_name = Scheem_name
if st.session_state.selected_division is not None:
    selected_name = st.session_state.selected_division


# piechart = cm.crop_area_piechart(dfca)
piechart, titlepie = cm.plotly_pie_chart(
    dfca, selected_name, selected_season_year, selected_season
)
titlepie = title = (
    f'<p style="font:Courier; color:gray; font-size: 20px;">{titlepie}</p>'
)
...
with col[0]:
    ...
    # st.write(map_data)
    st.write("")
    st.markdown(title_line_chart, unsafe_allow_html=True)
...
with col[1]:
    ...
    st.plotly_chart(piechart, use_container_width=False)
    chart, title = cm.alt_bar_chart(
        df_map, selected_indicator, col_name, selected_season_year, crop_season
    )
